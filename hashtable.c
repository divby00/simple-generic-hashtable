...
static uint16_t hashtable_pearson_hash_function(char* str, size_t len) {
    size_t i;
...
static uint16_t hashtable_bkdr_hash_function(char* str, size_t len) {
    uint16_t hash = 0;
    size_t i = 0;
    for(i=0; i<len; str++, i++) {
...
static HASHTABLE_ENTRY* hashtable_entry_create(char* key, void* params_create, void* (*create)(void *params), void (*destroy)(void *params)) {
...
static void hashtable_remove(HASHTABLE* hashtable, char* key) {
    uint16_t hashcode = 0;
    HASHTABLE_ENTRY* entry = NULL;
    HASHTABLE_ENTRY* first = NULL;
...
static void hashtable_put(HASHTABLE* hashtable, char* key, void* params_create, void* (*create)(void *params), void (*destroy)(void *params)) {
    uint16_t hashcode = 0;
    HASHTABLE_ENTRY* entry = NULL;
...
static uint8_t hashtable_contains(HASHTABLE *hashtable, char *key) {
    return (hashtable->get(hashtable, key) != NULL);
}


static void* hashtable_get(HASHTABLE* hashtable, char* key) {
    uint16_t hashcode = 0;
    HASHTABLE_ENTRY* entry = NULL;
    hashcode = hashtable->hash(key, strlen(key));
...
static HASHTABLE* hashtable_create(enum HASHTABLE_SIZE size) {
    ...
    if (hashtable != NULL) {
        ...
        if (size == HT_SIZE_NORMAL) {
            ...
            hashtable->hash = hashtable_bkdr_hash_function;
        } else {
            hashtable->size = UINT8_MAX + 1;
            hashtable->hash = hashtable_pearson_hash_function;
        }

        hashtable->table = malloc(sizeof(HASHTABLE_ENTRY*) * hashtable->size);
        if (hashtable->table != NULL) {
            memset(hashtable->table, 0, sizeof(HASHTABLE_ENTRY*) * hashtable->size);
...
HASHTABLE* hashtable_init_small(void) {
    return hashtable_create(HT_SIZE_SMALL);
}


void hashtable_quit(HASHTABLE* hashtable) {
    HASHTABLE_ENTRY* entry = NULL;
    HASHTABLE_ENTRY* aux = NULL;
    size_t i = 0;
